...
with app.app_context():
    ...
    def profile():
        ...
        if add_photo_form.validate_on_submit():
            url = add_photo_form.url.data
            comment = add_photo_form.comment.data
            new_pic = Picture(url, comment)
            with app.app_context():
                db.session.add(new_pic)
                db.session.commit()
            return render_template("profile.html", url=url, comment=comment)
        else:
            flash("Picture not added!")
            return render_template("profile.html", add_photo_form=add_photo_form)
